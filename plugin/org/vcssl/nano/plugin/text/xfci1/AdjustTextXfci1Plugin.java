...
public class AdjustTextXfci1Plugin implements ExternalFunctionConnectorInterface1 {
	...
	public Object invoke(Object[] arguments) throws ConnectorException {
		...
		String text = (String)arguments[0]; // 引数: 変換元のテキスト
		long mode = (long)arguments[1];     // 引数: 変換モード
		String result = null;               // 戻り値: 変換後のテキスト

		if (mode == TextModeConstants.TRIM) {
			result = text.trim();

		} else if (mode == TextModeConstants.LOWER_CASE) {
			result = text.toLowerCase();

		} else if (mode == TextModeConstants.UPPER_CASE) {
			result = text.toUpperCase();

		} else {
			String errorMessage = this.isJapanese ?
					"サポートされていないモードの値が指定されました: " + mode:
					"The specified mode value is unsupported: " + mode;
